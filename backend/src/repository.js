...
export const repository={
 ...
 async saveRuntimeState(roundId,payload){
  ...
  const x=await query(`INSERT INTO round_runtime_state(round_id,state,feedback,admin_log,revision) VALUES($1,$2,$3,$4,1) ON CONFLICT(round_id) DO UPDATE SET state=EXCLUDED.state,feedback=EXCLUDED.feedback,revision=round_runtime_state.revision+1,updated_at=now() WHERE round_runtime_state.revision=$5 RETURNING state,feedback,admin_log "adminLog",revision,updated_at "updatedAt"`,[roundId,payload.state||{},payload.feedback||[],payload.adminLog||[],expected]);
 ...
 async appendAdminLog(roundId,item){const x=await query(`INSERT INTO round_runtime_state(round_id,state,feedback,admin_log,revision) VALUES($1,'{}'::jsonb,'[]'::jsonb,jsonb_build_array($2::jsonb),1) ON CONFLICT(round_id) DO UPDATE SET admin_log=jsonb_build_array($2::jsonb)||round_runtime_state.admin_log,revision=round_runtime_state.revision+1,updated_at=now() RETURNING revision`,[roundId,JSON.stringify(item)]);return x.rows[0]},
 async clearAdminLog(roundId){const x=await query(`UPDATE round_runtime_state SET admin_log='[]'::jsonb,revision=revision+1,updated_at=now() WHERE round_id=$1 RETURNING revision`,[roundId]);return x.rows[0]||{revision:0}},
