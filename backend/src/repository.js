...
export const repository={
 ...
 async getRuntimeState(roundId){const x=await query('SELECT state,feedback,admin_log "adminLog",updated_at "updatedAt" FROM round_runtime_state WHERE round_id=$1',[roundId]);return x.rows[0]||null},
 async saveRuntimeState(roundId,payload){const x=await query(`INSERT INTO round_runtime_state(round_id,state,feedback,admin_log) VALUES($1,$2,$3,$4) ON CONFLICT(round_id) DO UPDATE SET state=EXCLUDED.state,feedback=EXCLUDED.feedback,admin_log=EXCLUDED.admin_log,updated_at=now() RETURNING state,feedback,admin_log "adminLog",updated_at "updatedAt"`,[roundId,payload.state||{},payload.feedback||[],payload.adminLog||[]]);return x.rows[0]},
 ...
 ,async saveTeam(roundId,team){
  ...
  try{
   ...
   const selection={riders:String(team.riders||""),reserves:String(team.reserves||"")};
